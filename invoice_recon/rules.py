...
AUDIT_RULES_HELP = (
    "审计与异常追踪命令组。\n\n"
    "每次导入、匹配、复核、撤销、导出、快照恢复和包导入都会留有可查询的操作记录，\n"
    "持久化到 SQLite，跨重启可查。\n\n"
    "子命令: list / show / export / config"
)

AUDIT_LIST_HELP = (
    "查询审计记录。\n\n"
    "支持按批次、操作者、时间范围、动作类型和结果筛选。"
)

AUDIT_SHOW_HELP = (
    "查看单条审计记录的完整详情。\n\n"
    "包含关联的规则版本、文件落点、冲突原因、撤销前后状态和导出结果。"
)

AUDIT_EXPORT_HELP = (
    "导出审计报告。\n\n"
    "支持 CSV 和 JSON 格式。筛选条件同 audit list。\n\n"
    "【导出规则】\n"
    "  - 目标文件已存在时报错不覆盖，不写半截文件；\n"
    "  - 目标目录不可写时报错；\n"
    "  - 关联数据缺失时在报告中标注。"
)
...
AUDIT_CONFIG_HELP = (
    "查看或修改审计配置。\n\n"
    "  --retention-days: 审计记录保留天数（默认 365，0 表示永久保留）\n"
    "  --verbose / --no-verbose: 是否记录详细字段（如文件落点、冲突原因等）\n\n"
    "非法配置会被拒绝（如 retention_days 为负数）。"
)

AUDIT_OK_EXPORTED = "审计报告已导出"
AUDIT_ERR_EXPORT_FILE_EXISTS = "目标文件已存在，不会覆盖"
AUDIT_ERR_EXPORT_DIR_NOT_FOUND = "目标目录不存在"
AUDIT_ERR_EXPORT_DIR_NOT_WRITABLE = "目标目录不可写"
AUDIT_ERR_EXPORT_NO_RECORDS = "无匹配的审计记录"

AUDIT_CONFIG_RETENTION_DAYS_LABEL = "保留天数"
AUDIT_CONFIG_VERBOSE_LABEL = "记录详细字段"
AUDIT_CONFIG_SAVED = "审计配置已更新"
...
REPLAY_RULES_HELP = (
    ...
    "子命令: start / list / show / export / import / undo / config / verify"
)

REPLAY_START_HELP = (
    "开始一个回放会话。\n\n"
    "会话会记录输入摘要、配置快照，之后可通过 add_step 添加步骤。"
)

REPLAY_LIST_HELP = (
    "列出回放会话。\n\n"
    "支持按批次、操作者、结果、动作、时间范围筛选。"
)

REPLAY_SHOW_HELP = (
    "查看回放会话详情。\n\n"
    "包含会话信息、配置快照、输入摘要和所有步骤。"
)

REPLAY_EXPORT_HELP = (
    "导出回放证据包。\n\n"
    "支持 JSON / CSV / ZIP 三种格式。\n\n"
    "【导出规则】\n"
    "  - 目标文件已存在时报错不覆盖，不写半截文件；\n"
    "  - 目标目录不可写时报错；\n"
    "  - ZIP 格式包含 manifest、sessions、steps 和校验和。"
)
...
REPLAY_IMPORT_HELP = (
    ...
    "【导入规则】\n"
    "  - 导入前校验包完整性，不完整则拒绝导入；\n"
    "  - 版本不兼容时报错，使用 --force 可强制导入；\n"
    "  - 重复会话（session_key 相同）默认跳过，使用 --force 会覆盖；\n"
    "  - 不覆盖已有数据，始终作为新记录导入。"
)

REPLAY_UNDO_HELP = (
    "撤销回放会话。\n\n"
    "标记会话为 undone 状态，保留记录但标记为已撤销，可回溯查看。"
)

REPLAY_CONFIG_HELP = (
    "查看或修改回放配置。\n\n"
    "  --detail / --no-detail: 是否开启明细采集（默认开启）\n"
    "  --masked-fields: 脱敏字段列表，逗号分隔\n"
    "  --retention-days: 回放记录保留天数（默认 365，0 表示永久保留）\n\n"
    "非法配置会被拒绝并写清原因。"
)

REPLAY_VERIFY_HELP = (
    "校验证据包完整性。\n\n"
    "检查必需文件、校验和、格式和 schema 版本兼容性。"
)

REPLAY_OK_STARTED = "回放会话已开始"
REPLAY_OK_EXPORTED = "回放证据包已导出"
REPLAY_OK_IMPORTED = "回放证据包已导入"
REPLAY_OK_UNDONE = "回放会话已撤销"
REPLAY_OK_VERIFIED = "证据包校验通过"

REPLAY_CONFIG_DETAIL_LABEL = "明细采集"
REPLAY_CONFIG_MASKED_LABEL = "脱敏字段"
REPLAY_CONFIG_RETENTION_LABEL = "保留天数"
REPLAY_CONFIG_SAVED = "回放配置已更新"
...
DRILL_END_HELP = (
    "结束当前演练。\n\n"
    "收口成 success/failure/error，完成回放记录。"
)

DRILL_UNDO_HELP = (
    "撤销当前演练。\n\n"
    "标记为 undone 状态，保留完整操作链路，用于回溯。"
)

DRILL_STATUS_HELP = (
    "查看当前演练状态。\n\n"
    "显示当前活动演练的会话信息、已执行步骤数和当前批次。"
)
...
PUBLIC_CONTRACTS: List[str] = [
    ...
    "审计：所有操作（导入/匹配/复核/撤销/导出/快照恢复/包导入/配置）都留有可查询记录",
    "审计持久性：跨重启后仍可查询审计记录",
    "审计配置：可调整保留天数和详细字段开关，非法配置被拒绝",
    "审计导出：支持 CSV/JSON，目标文件已存在/目录不可写时报错不写半截",
    "任务回放：可记录输入、配置、关键步骤、冲突处理、撤销结果和异常日志",
    "回放持久性：跨重启后仍可按动作、结果、时间、操作者和批次筛选",
    "回放配置：支持明细采集、脱敏字段、保留天数，非法配置被拦住并写清原因",
    "回放导出：支持 JSON/CSV/压缩包，同名文件或权限不足不留下半截产物",
    "回放导入：处理版本不兼容、重复批次、缺少文件和只读目录，不覆盖已有数据",
    "回放撤销：可撤销回放会话，撤销后仍可回看，保留完整操作链路",
    "操作录制：drill begin 后，导入/匹配/复核/撤销/导出自动挂到同一条回放轨迹",
    "操作录制收口：演练结束自动标记 success/failure/undone，每步含输入、配置、批次、异常",
    "操作录制持久性：跨重启后仍可按时间段、批次、动作和结论查询演练记录",
]
